...
if [ $errcode -eq 0 ]; then
  $boundaryvtk -loadvtk $dirout/${name}__Actual.vtk -filexml $dirout/${name}.xml -motiondata $dirout -savevtkdata $dirout/Box -onlymk:11
  errcode=$?
fi

if [ $errcode -eq 0 ]; then
  $boundaryvtk -loadvtk $dirout/${name}__Actual.vtk -filexml $dirout/${name}.xml -motiondata $dirout -savevtkdata $dirout/MotionFloating -onlymk:62-79 
  errcode=$?
fi
