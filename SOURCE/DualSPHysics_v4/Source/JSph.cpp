...
void JSph::InitVars() {
    ...
    TVisco = VISCO_None;
    TDeltaSph = DELTA_None;
    DeltaSph = 0;
    TShifting = SHIFT_None;
    ShiftCoef = ShiftTFS = 0;
    Visco = 0;
    ViscoBoundFactor = 1;
    UseDEM = false;  //(DEM)
    DemDtForce = 0;  //(DEM)
...
void JSph::ConfigDomainFixed(tdouble3 vmin, tdouble3 vmax) {
    ...
    CfgDomainFixedMin = vmin;
    CfgDomainFixedMax = vmax;
}
...
void JSph::ConfigDomainParticles(tdouble3 vmin, tdouble3 vmax) {
    CfgDomainParticles = true;
    CfgDomainParticlesMin = vmin;
    CfgDomainParticlesMax = vmax;
}
...
void JSph::ConfigDomainParticlesPrc(tdouble3 vmin, tdouble3 vmax) {
    CfgDomainParticles = true;
    CfgDomainParticlesPrcMin = vmin;
    CfgDomainParticlesPrcMax = vmax;
}
...
void JSph::LoadConfig(const JCfgRun *cfg) {
    ...
    // 输出配置
    // 输出格式(0: Node, 1: bi2, 2: vtk, 4: csv, 8: info)
    ...
    if (TDeltaSph == DELTA_Dynamic && Cpu)
        TDeltaSph = DELTA_DynamicExt; //-It is necessary because the interaction is divided in two steps: fluid-fluid/float and fluid-bound.
    ...
    if (cfg->Shifting >= 0) {
        switch (cfg->Shifting) {
            case 0:
                ...
                break;
            case 1:
                TShifting = SHIFT_NoBound;
                break;
            case 2:
                TShifting = SHIFT_NoFixed;
                break;
            case 3:
                TShifting = SHIFT_Full;
                break;
            default:
                RunException(met, "Shifting mode is not valid.");
        }
    ...
    if (cfg->RhopOutModif) {
        RhopOutMin = cfg->RhopOutMin;
        RhopOutMax = cfg->RhopOutMax;
    }
...
void JSph::LoadCaseConfig() {
    ...
    switch (eparms.GetValueInt("PosDouble", true, 0)) {
        ...
        case 2:
            ...
            break;
        default:
            RunException(met, "PosDouble value is not valid.");
    }
    switch (eparms.GetValueInt("RigidAlgorithm", true, 1)) { //(DEM)
        ...
        case 2:
            ...
            break;
        default:
            RunException(met, "Rigid algorithm is not valid.");
    }
    switch (eparms.GetValueInt("StepAlgorithm", true, 1)) {
        case 1:
            ...
            break;
        case 2:
            TStep = STEP_Symplectic;
            break;
        default:
            RunException(met, "Step algorithm is not valid.");
    }
    VerletSteps = eparms.GetValueInt("VerletSteps", true, 40);
    switch (eparms.GetValueInt("Kernel", true, 2)) {
        case 1:
            TKernel = KERNEL_Cubic;
            break;
        case 2:
            TKernel = KERNEL_Wendland;
            break;
        default:
            RunException(met, "Kernel choice is not valid.");
    }
    switch (eparms.GetValueInt("ViscoTreatment", true, 1)) {
        ...
        case 2:
            ...
            break;
        default:
            RunException(met, "Viscosity treatment is not valid.");
    }
    ...
    if (eparms.Exists("XPeriodicIncY")) {
        PeriXinc.y = eparms.GetValueDouble("XPeriodicIncY");
        PeriX = true;
    }
    if (eparms.Exists("XPeriodicIncZ")) {
        PeriXinc.z = eparms.GetValueDouble("XPeriodicIncZ");
        PeriX = true;
    }
    if (eparms.Exists("YPeriodicIncX")) {
        PeriYinc.x = eparms.GetValueDouble("YPeriodicIncX");
        PeriY = true;
    }
    if (eparms.Exists("YPeriodicIncZ")) {
        PeriYinc.z = eparms.GetValueDouble("YPeriodicIncZ");
        PeriY = true;
    }
    if (eparms.Exists("ZPeriodicIncX")) {
        PeriZinc.x = eparms.GetValueDouble("ZPeriodicIncX");
        PeriZ = true;
    }
    if (eparms.Exists("ZPeriodicIncY")) {
        PeriZinc.y = eparms.GetValueDouble("ZPeriodicIncY");
        PeriZ = true;
    }
    if (eparms.Exists("XYPeriodic")) {
        PeriXY = PeriX = PeriY = true;
        PeriXZ = PeriYZ = false;
        PeriXinc = PeriYinc = TDouble3(0);
    }
    if (eparms.Exists("XZPeriodic")) {
        PeriXZ = PeriX = PeriZ = true;
        PeriXY = PeriYZ = false;
        PeriXinc = PeriZinc = TDouble3(0);
    }
    if (eparms.Exists("YZPeriodic")) {
        PeriYZ = PeriY = PeriZ = true;
        PeriXY = PeriXZ = false;
        PeriYinc = PeriZinc = TDouble3(0);
    }
    ...
    if (eparms.Exists("DomainParticles")) {
        ...
        ConfigDomainParticles(TDouble3(eparms.GetValueNumDouble(key, 0), eparms.GetValueNumDouble(key, 1),
                                       eparms.GetValueNumDouble(key, 2)),
                              TDouble3(eparms.GetValueNumDouble(key, 3), eparms.GetValueNumDouble(key, 4),
                                       eparms.GetValueNumDouble(key, 5)));
    }
    if (eparms.Exists("DomainParticlesPrc")) {
        string key = "DomainParticlesPrc";
        ConfigDomainParticlesPrc(TDouble3(eparms.GetValueNumDouble(key, 0), eparms.GetValueNumDouble(key, 1),
                                          eparms.GetValueNumDouble(key, 2)),
                                 TDouble3(eparms.GetValueNumDouble(key, 3), eparms.GetValueNumDouble(key, 4),
                                          eparms.GetValueNumDouble(key, 5)));
    }
    if (eparms.Exists("DomainFixed")) {
        ...
        ConfigDomainFixed(TDouble3(eparms.GetValueNumDouble(key, 0), eparms.GetValueNumDouble(key, 1),
                                   eparms.GetValueNumDouble(key, 2)),
                          TDouble3(eparms.GetValueNumDouble(key, 3), eparms.GetValueNumDouble(key, 4),
                                   eparms.GetValueNumDouble(key, 5)));
    }
    ...
    if (MotionObjCount) {
        ...
        if (int(MotionObjCount) < Motion->Init(&xml, "case.execution.motion", DirCase))
    ...
    if (UseDEM) {
        ...
        for (unsigned c = 0; c < parts.CountBlocks(); c++) {
            ...
            if (block.Type != PT_Fluid) {
                ...
                if (!block.ExistsSubValue("Young_Modulus", "value"))
                    RunException(met, fun::PrintStr("Object mk=%u - Value of Young_Modulus is invalid.",
                                                    block.GetMk()));
                if (!block.ExistsSubValue("PoissonRatio", "value"))
                    RunException(met, fun::PrintStr("Object mk=%u - Value of PoissonRatio is invalid.", block.GetMk()));
                if (!block.ExistsSubValue("Kfric", "value"))
                    RunException(met, fun::PrintStr("Object mk=%u - Value of Kfric is invalid.", block.GetMk()));
                if (!block.ExistsSubValue("Restitution_Coefficient", "value"))
                    RunException(met, fun::PrintStr("Object mk=%u - Value of Restitution_Coefficient is invalid.",
                                                    block.GetMk()));
                ...
                DemObjs[tav].tau = (DemObjs[tav].young ? (1 - DemObjs[tav].poisson * DemObjs[tav].poisson) /
                                                         DemObjs[tav].young : 0);
                ...
                if (block.ExistsValue("Restitution_Coefficient_User"))
                    DemObjs[tav].restitu = block.GetValueFloat("Restitution_Coefficient_User");
            }
        }
    }
...
void JSph::VisuDemCoefficients() const {
    ...
    for (unsigned c = 0; c < MkListSize; c++) {
        ...
        if (type == CODE_TYPE_FIXED || type == CODE_TYPE_MOVING || type == CODE_TYPE_FLOATING) {
            Log->Printf("  Object %s  mkbound:%u  mk:%u",
                        (type == CODE_TYPE_FIXED ? "Fixed" : (type == CODE_TYPE_MOVING ? "Moving" : "Floating")),
                        MkList[c].mktype, MkList[c].mk);
...
void JSph::LoadMkInfo(const JSpaceParts *parts) {
    ...
    for (unsigned c = 0; c < MkListSize; c++) {
        ...
        switch (block.Type) {
            case PT_Fixed:
                MkList[c].code = CodeSetType(0, PART_BoundFx, c);
                break;
            case PT_Moving:
                MkList[c].code = CodeSetType(0, PART_BoundMv, c - MkListFixed);
                break;
            case PT_Floating:
                MkList[c].code = CodeSetType(0, PART_BoundFt, c - MkListFixed - MkListMoving);
                break;
            case PT_Fluid:
                MkList[c].code = CodeSetType(0, PART_Fluid, c - MkListBound);
                break;
        }
    }
}
...
void JSph::LoadCodeParticles(unsigned np, const unsigned *idp, word *code) const {
    ...
    for (unsigned p = 0; p < np; p++) {
        ...
        if (id < finfixed)cod = CodeSetType(cod, PART_BoundFx, cmk);
            ...
            if (cmk - MkListFixed - MkListMoving >= FtCount)
                RunException(met, "Floating code of particles was not found.");
        } else {
...
void JSph::ResizeMapLimits() {
    ...
    if (dmin.x > MapRealPosMin.x || dmin.y > MapRealPosMin.y || dmin.z > MapRealPosMin.z || dmax.x < MapRealPosMax.x ||
        dmax.y < MapRealPosMax.y || dmax.z < MapRealPosMax.z)
        RunException("ResizeMapLimits", "Domain limits is not valid.");
    if (!PeriX) {
        MapRealPosMin.x = dmin.x;
        MapRealPosMax.x = dmax.x;
    }
    if (!PeriY) {
        MapRealPosMin.y = dmin.y;
        MapRealPosMax.y = dmax.y;
    }
    if (!PeriZ) {
        MapRealPosMin.z = dmin.z;
        MapRealPosMax.z = dmax.z;
    }
}
    ...
    for (unsigned p = 0; p < n; p++) {
        ...
        if (codeout < CODE_OUTIGNORE) {
            ...
            if (ps >= DomRealPosMin && ps < DomRealPosMax) {
                ...
                RunException(met,
                             "Found new particles out."); //-No puede haber nuevas particulas excluidas. //-There can not be new particles excluded.
                dcell[p] = PC__CodeOut;
            }
...
void JSph::ConfigCellOrder(TpCellOrder order, unsigned np, tdouble3 *pos, tfloat4 *velrhop) {
    ...
    if (Simulate2D && CellOrder != ORDER_XYZ && CellOrder != ORDER_ZYX)
        RunException("ConfigCellOrder", "In 2D simulations the value of CellOrder must be XYZ or ZYX.");
    ...
    if (CellOrder != ORDER_XYZ) {
        ...
        if (v.x == 2) {
            PeriX = periy;
            PeriXinc = OrderCode(periyinc);
        }
        if (v.x == 3) {
            PeriX = periz;
            PeriXinc = OrderCode(perizinc);
        }
        if (v.y == 1) {
            PeriY = perix;
            PeriYinc = OrderCode(perixinc);
        }
        if (v.y == 3) {
            PeriY = periz;
            PeriYinc = OrderCode(perizinc);
        }
        if (v.z == 1) {
            PeriZ = perix;
            PeriZinc = OrderCode(perixinc);
        }
        if (v.z == 2) {
            PeriZ = periy;
            PeriZinc = OrderCode(periyinc);
        }
        if (perixy) {
...
void JSph::ConfigCellDivision() {
    if (CellMode != CELLMODE_2H && CellMode != CELLMODE_H)
        RunException("ConfigCellDivision", "The CellMode is invalid.");
    Hdiv = (CellMode == CELLMODE_2H ? 1 : 2);
    Scell = Dosh / Hdiv;
    MovLimit = Scell * 0.9f;
    Map_Cells = TUint3(unsigned(ceil(Map_Size.x / Scell)), unsigned(ceil(Map_Size.y / Scell)),
...
void JSph::SelecDomain(tuint3 celini, tuint3 celfin) {
    ...
    if (DomCelIni.x >= Map_Cells.x || DomCelIni.y >= Map_Cells.y || DomCelIni.z >= Map_Cells.z)
        RunException(met, "DomCelIni is invalid.");
    if (DomCelFin.x > Map_Cells.x || DomCelFin.y > Map_Cells.y || DomCelFin.z > Map_Cells.z)
        RunException(met, "DomCelFin is invalid.");
    ...
    if (!DomCellCode)
        RunException(met, string("Failed to select a valid CellCode for ") + fun::UintStr(DomCells.x) + "x" +
                          fun::UintStr(DomCells.y) + "x" + fun::UintStr(DomCells.z) + " cells (CellMode=" +
                          GetNameCellMode(CellMode) + ").");
    //-Prints configurantion.
    Log->Print(string("DomCells=(") + fun::Uint3Str(OrderDecode(DomCells)) + ")");
    Log->Print(fun::VarStr("DomCellCode",
                           fun::UintStr(PC__GetSx(DomCellCode)) + "_" + fun::UintStr(PC__GetSy(DomCellCode)) + "_" +
                           fun::UintStr(PC__GetSz(DomCellCode))));
}
...
unsigned JSph::CalcCellCode(tuint3 ncells) {
    unsigned sxmin = 2;
    for (; ncells.x >> sxmin; sxmin++);
    unsigned symin = 2;
    for (; ncells.y >> symin; symin++);
    unsigned szmin = 2;
    for (; ncells.z >> szmin; szmin++);
...
void JSph::CalcFloatingRadius(unsigned np, const tdouble3 *pos, const unsigned *idp) {
    ...
    if (PeriX && fabs(PeriXinc.x) <= radiusmax)
        RunException(met, fun::PrintStr("The floating radius (%g) is too large for periodic distance in X (%g).",
                                        radiusmax, abs(PeriXinc.x)));
    if (PeriY && fabs(PeriYinc.y) <= radiusmax)
        RunException(met, fun::PrintStr("The floating radius (%g) is too large for periodic distance in Y (%g).",
                                        radiusmax, abs(PeriYinc.y)));
    if (PeriZ && fabs(PeriZinc.z) <= radiusmax)
        RunException(met, fun::PrintStr("The floating radius (%g) is too large for periodic distance in Z (%g).",
                                        radiusmax, abs(PeriZinc.z)));
}
...
tdouble3 JSph::UpdatePeriodicPos(tdouble3 ps) const {
    ...
    const bool out = (dx != dx || dy != dy || dz != dz || dx < 0 || dy < 0 || dz < 0 || dx >= MapRealSize.x ||
...
void JSph::PrintSizeNp(unsigned np, llong size) const {
    Log->Printf("**Requested %s memory for %u particles: %.1f MB.", (Cpu ? "cpu" : "gpu"), np,
...
void JSph::ConfigSaveData(unsigned piece, unsigned pieces, std::string div) {
    ...
    if (SvData & SDAT_Info || SvData & SDAT_Binx) {
        ...
        DataBi4->ConfigParticles(CaseNp, CaseNfixed, CaseNmoving, CaseNfloat, CaseNfluid, CasePosMin, CasePosMax,
                                 NpDynamic, ReuseIds);
    ...
    if (SvData & SDAT_Binx) {
        ...
        DataOutBi4->ConfigLimits(OrderDecode(MapRealPosMin), OrderDecode(MapRealPosMax), (RhopOut ? RhopOutMin : 0),
                                 (RhopOut ? RhopOutMax : 0));
    ...
    if (SvData & SDAT_Binx && FtCount) {
        ...
        for (unsigned cf = 0; cf < FtCount; cf++)
            DataFloatBi4->AddHeadData(cf, FtObjs[cf].mkbound, FtObjs[cf].begin, FtObjs[cf].count, FtObjs[cf].mass,
                                      FtObjs[cf].radius);
...
//==============================================================================
void
JSph::AddParticlesOut(unsigned nout, const unsigned *idp, const tdouble3 *pos, const tfloat3 *vel, const float *rhop,
                      unsigned noutrhop, unsigned noutmove) {
...
void JSph::SavePartData(unsigned npok, unsigned nout, const unsigned *idp, const tdouble3 *pos, const tfloat3 *vel,
    ...
    if (DataBi4) {
        ...
        JBinaryData *bdpart = DataBi4->AddPartInfo(Part, TimeStep, npok, nout, Nstep,
                                                   TimerPart.GetElapsedTimeD() / 1000., vdom[0], vdom[1], TotalNp);
        ...
        if (SvData & SDAT_Binx) {
            ...
            if (0) {//-Example saving a new array (Pressure) in files BI4.
                ...
                for (unsigned p = 0; p < npok; p++)
                    press[p] = (idp[p] >= CaseNbound ? CteB * (pow(rhop[p] / RhopZero, Gamma) - 1.0f) : 0.f);
                DataBi4->AddPartData("Pressure", npok, press);
            }
            DataBi4->SaveFilePart();
            delete[] press;
            press = NULL;//-Memory must to be deallocated after saving file because DataBi4 uses this memory space.
        }
    ...
    if ((SvData & SDAT_Csv) || (SvData & SDAT_Vtk)) {
        ...
        unsigned nfields = 0;
        if (idp) {
            fields[nfields] = JFormatFiles2::DefineField("Idp", JFormatFiles2::UInt32, 1, idp);
            nfields++;
        }
        if (vel) {
            fields[nfields] = JFormatFiles2::DefineField("Vel", JFormatFiles2::Float32, 3, vel);
            nfields++;
        }
        if (rhop) {
            fields[nfields] = JFormatFiles2::DefineField("Rhop", JFormatFiles2::Float32, 1, rhop);
            nfields++;
        }
        if (type) {
            fields[nfields] = JFormatFiles2::DefineField("Type", JFormatFiles2::UChar8, 1, type);
            nfields++;
        }
        if (SvData & SDAT_Vtk)
    ...
    if (DataOutBi4 && PartsOut->GetCount()) {
        if (SvDouble)
            DataOutBi4->SavePartOut(Part, TimeStep, PartsOut->GetCount(), PartsOut->GetIdpOut(), PartsOut->GetPosOut(),
                                    PartsOut->GetVelOut(), PartsOut->GetRhopOut());
        else {
            const tfloat3 *posf3 = GetPointerDataFloat3(PartsOut->GetCount(), PartsOut->GetPosOut());
            DataOutBi4->SavePartOut(Part, TimeStep, PartsOut->GetCount(), PartsOut->GetIdpOut(), posf3,
                                    PartsOut->GetVelOut(), PartsOut->GetRhopOut());
            delete[] posf3;
        }
    }
    ...
    if (DataFloatBi4) {
        if (CellOrder == ORDER_XYZ)
            for (unsigned cf = 0; cf < FtCount; cf++)
                DataFloatBi4->AddPartData(cf, FtObjs[cf].center, FtObjs[cf].fvel, FtObjs[cf].fomega);
        else
            for (unsigned cf = 0; cf < FtCount; cf++)
                DataFloatBi4->AddPartData(cf, OrderDecodeValue(CellOrder, FtObjs[cf].center),
                                          OrderDecodeValue(CellOrder, FtObjs[cf].fvel), OrderDecodeValue(CellOrder,
                                                                                                         FtObjs[cf].fomega));
...
void JSph::SaveData(unsigned npok, const unsigned *idp, const tdouble3 *pos, const tfloat3 *vel, const float *rhop,
    ...
    if (Part > PartIni || Nstep) {
        ...
        Log->Printf("Part%s  %12.6f  %12d  %7d  %9.2f  %14s", suffixpartx.c_str(), TimeStep, (Nstep + 1),
...
JSph::GetResInfo(float tsim, float ttot, const std::string &headplus, const std::string &detplus, std::string &hinfo,
                 std::string &dinfo) {
    hinfo = hinfo +
            "#RunName;RunCode;DateTime;Np;TSimul;TSeg;TTotal;MemCpu;MemGpu;Steps;PartFiles;PartsOut;MaxParticles;MaxCells;Hw;StepAlgo;Kernel;Viscosity;ViscoValue;DeltaSPH;TMax;Nbound;Nfixed;H;RhopOut;PartsRhopOut;PartsVelOut;CellMode" +
            headplus;
    ...
    dinfo = dinfo + ";" + Hardware + ";" + GetStepName(TStep) + ";" + GetKernelName(TKernel) + ";" +
            GetViscoName(TVisco) + ";" + fun::FloatStr(Visco);
    ...
    dinfo = dinfo + ";" + rhopcad + ";" + fun::UintStr(GetOutRhopCount()) + ";" + fun::UintStr(GetOutMoveCount()) +
...
void JSph::ShowResume(bool stop, float tsim, float ttot, bool all, std::string infoplus) {
    ...
    Log->Printf("CPU Memory.......................: %lld (%.2f MB)", MaxMemoryCpu,
                double(MaxMemoryCpu) / (1024 * 1024));
    if (MaxMemoryGpu)
        Log->Printf("GPU Memory.......................: %lld (%.2f MB)", MaxMemoryGpu,
                    double(MaxMemoryGpu) / (1024 * 1024));
}
