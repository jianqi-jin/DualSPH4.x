...
JSpaceCtes::StConstants JSpaceCtes::CalcConstans(StConstants cte) {
    ...
    const double mg = sqrt(
            (cte.gravity.x * cte.gravity.x) + (cte.gravity.y * cte.gravity.y) + (cte.gravity.z * cte.gravity.z));
...
void JSpaceCtes::LoadDefault() {
    ...
    SetHSwlAuto(true);
    SetHSwl(0);
    SetSpeedSystemAuto(true);
    SetSpeedSystem(0);
    SetCoefSound(10);
    SetSpeedSoundAuto(true);
    SetSpeedSound(0);
    SetCoefH(0.866025);
    SetGamma(7);
    SetRhop0(1000);
    ...
    SetBAuto(true);
    SetB(0);
    SetMassBoundAuto(true);
    SetMassBound(0);
    SetMassFluidAuto(true);
    SetMassFluid(0);
}
...
//==============================================================================
void JSpaceCtes::ReadXmlElementAuto(JXml *sxml, TiXmlElement *node, bool optional, std::string name, double &value,
                                    bool &valueauto) {
...
void JSpaceCtes::ReadXmlDef(JXml *sxml, TiXmlElement *node) {
    ...
    if (ch != 0 && chdp != 0)
        RunException(met, "Only one constant must be defined (coefh or hdp).", sxml->ErrGetFileRow(node));
    if (ch)SetCoefH(ch);
...
//==============================================================================
void JSpaceCtes::WriteXmlElementAuto(JXml *sxml, TiXmlElement *node, std::string name, double value, bool valueauto,
                                     std::string comment, std::string unitscomment) const {
...
void JSpaceCtes::WriteXmlDef(JXml *sxml, TiXmlElement *node) const {
    ...
    WriteXmlElementComment(JXml::AddElementDouble3(node, "gravity", GetGravity()), "Gravitational acceleration",
                           "m/s^2");
    WriteXmlElementComment(JXml::AddElementAttrib(node, "rhop0", "value", GetRhop0()), "Reference density of the fluid",
                           "kg/m^3");
    WriteXmlElementAuto(sxml, node, "hswl", GetHSwl(), GetHSwlAuto(),
                        "Maximum still water level to calculate speedofsound using coefsound", "metres (m)");
    WriteXmlElementComment(JXml::AddElementAttrib(node, "gamma", "value", GetGamma()),
                           "Polytropic constant for water used in the state equation");
    WriteXmlElementAuto(sxml, node, "speedsystem", GetSpeedSystem(), GetSpeedSystemAuto(),
                        "Maximum system speed (by default the dam-break propagation is used)");
    WriteXmlElementComment(JXml::AddElementAttrib(node, "coefsound", "value", GetCoefSound()),
                           "Coefficient to multiply speedsystem");
    WriteXmlElementAuto(sxml, node, "speedsound", GetSpeedSound(), GetSpeedSoundAuto(),
                        "Speed of sound to use in the simulation (by default speedofsound=coefsound*speedsystem)");
    if (GetCoefH() || !GetCoefHdp())
        WriteXmlElementComment(JXml::AddElementAttrib(node, "coefh", "value", GetCoefH()),
                               "Coefficient to calculate the smoothing length (h=coefh*sqrt(3*dp^2) in 3D)");
    if (GetCoefHdp())
        WriteXmlElementComment(JXml::AddElementAttrib(node, "hdp", "value", GetCoefHdp()),
                               "Coefficient to calculate the smoothing length (hdp=h/dp)");
    WriteXmlElementComment(JXml::AddElementAttrib(node, "cflnumber", "value", GetCFLnumber()),
                           "Coefficient to multiply dt");
